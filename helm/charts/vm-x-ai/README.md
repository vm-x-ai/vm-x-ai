...
- **PostgreSQL**: Single instance with persistence (also stores `request_audit` for usage analytics)
...
2. **PostgreSQL**: Consider using a managed database service or PostgreSQL operator for HA (`request_audit` lives here too — size accordingly for usage analytics retention).
