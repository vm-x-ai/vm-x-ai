...
services:
  ...
  ui:
    ...
    environment:
      ...
      AUTH_OIDC_ISSUER: http://localhost:3000/api/oauth2
      AUTH_OIDC_CLIENT_ID: ui
      AUTH_OIDC_CLIENT_SECRET: ui
      AUTH_REDIRECT_PROXY_URL: http://localhost:3001/api/auth

      # API (network mode: host). Note the `/api` suffix: the API serves
      # every HTTP route under the `BASE_PATH` configured below.
  ...
  api:
    ...
    environment:
      ...
      # All HTTP routes (including OIDC, OpenAPI, completion surfaces)
      # are mounted under this prefix. The container default is `/api`.
      BASE_PATH: /api
  ...
  redis:
    ...
    ports:
      ...
      - '17001:17001'
    volumes:
      - ./docker/redis-config/node1/redis.conf:/usr/local/etc/redis/redis.conf
  ...
  redis2:
    ...
    ports:
      ...
      - '17002:17002'
    volumes:
      - ./docker/redis-config/node2/redis.conf:/usr/local/etc/redis/redis.conf
    command: ['redis-server', '/usr/local/etc/redis/redis.conf']
    network_mode: host
  ...
  redis3:
    ...
    ports:
      ...
      - '17003:17003'
    volumes:
      - ./docker/redis-config/node3/redis.conf:/usr/local/etc/redis/redis.conf
    command: ['redis-server', '/usr/local/etc/redis/redis.conf']
    network_mode: host
...
- **UI** on port `3001`
- **API** on port `3000`, with every HTTP route mounted under `/api` (so the OpenAPI doc lives at `http://localhost:3000/api/docs` and the OIDC issuer at `http://localhost:3000/api/oauth2`)
- **PostgreSQL** on port `5440` (config + audit logs + usage analytics)
- **Redis cluster** on ports `7001` / `7002` / `7003`

The published `vmxai/api:latest` image runs the migrations bundled in [`packages/api/src/migrations/`](https://github.com/vm-x-ai/vm-x-ai/tree/main/packages/api/src/migrations) on startup, including the model-pricing seed (per-token costs for every supported model). On a fresh DB, give the API a few extra seconds to finish that one-time work.
...
VM-X AI exposes three upstream-compatible surfaces, all under the same workspace/environment-scoped prefix:

- **OpenAI Chat Completions** — `POST /api/v1/completion/{workspaceId}/{environmentId}/chat/completions`
- **OpenAI Responses** — `POST /api/v1/completion/{workspaceId}/{environmentId}/responses`
- **Anthropic Messages** — `POST /api/v1/completion/{workspaceId}/{environmentId}/anthropic/messages` (the Anthropic SDK's auto-appended `/anthropic/v1/messages` path is supported as an alias)

The gateway preserves each upstream's wire format verbatim — pick the surface that matches the SDK you already use. Routing, fallback, and per-resource provider selection are configured per AI Resource in the UI, so any of the three surfaces can target any provider as long as that provider implements the surface (see [`packages/api/src/ai-provider/`](https://github.com/vm-x-ai/vm-x-ai/tree/main/packages/api/src/ai-provider) for the converter classes — OpenAI, Anthropic, Gemini, Groq, Perplexity, and AWS Bedrock Converse / Invoke).
...
client = OpenAI(
    ...
    base_url=f"http://localhost:3000/api/v1/completion/{workspace_id}/{environment_id}"
)
...
The same `base_url` also serves `client.responses.create(...)` (OpenAI Responses surface).
...
const openai = new OpenAI({
  ...
  baseURL: `http://localhost:3000/api/v1/completion/${workspaceId}/${environmentId}`,
});
...
```bash
curl http://localhost:3000/api/v1/completion/{workspaceId}/{environmentId}/chat/completions \
...
client = Anthropic(
    api_key="your-vmx-api-key-here",
    # The Anthropic SDK auto-appends `/v1/messages`, so stop one segment short:
    base_url=f"http://localhost:3000/api/v1/completion/{workspace_id}/{environment_id}/anthropic",
)
...
:::tip Runnable end-to-end examples
The Python package at [`examples/api-completion/`](https://github.com/vm-x-ai/vm-x-ai/tree/main/examples/api-completion) mirrors every snippet from these docs against a live gateway. It includes a `setup` script that provisions a workspace, environment, AI connection, AI resource, and API key, then writes the IDs to `examples/api-completion/.env.local` for the runnable scenarios (`chat_completions.py`, `responses.py`, `anthropic_messages.py`, `web_search.py`, `vmx_envelope.py`). Use it as the canonical reference for the VMX envelope (`vmx.providerArgs`, `__vmx_passthrough`, `vmx.correlationId`, `vmx.metadata`).
:::
...
1. **Default Configuration** (`default.docker-compose.yml`)
   ...
   - PostgreSQL + single-node Redis (port `6379`, `REDIS_MODE=single`)
   - Libsodium encryption
   - Usage analytics served from Postgres `request_audit` (no separate time-series store)

2. **Redis Cluster Configuration** (`redis-cluster.docker-compose.yml`)

   - Same services as `default` but with the 3-node Redis cluster on ports `7001` / `7002` / `7003` (the configuration shown earlier on this page)
   - Recommended whenever you intend to mirror production cache topology

3. **OpenTelemetry Configuration** (`otel.docker-compose.yml`)
...
4. **AWS Services Configuration** (`aws.docker-compose.yml`)
...
| `BASE_PATH`                   | URL prefix every HTTP route mounts under             | `/api`                  |
