#!/usr/bin/env bash
# Provision the example workspace and write IDs / API key to
# `examples/claude-agent-sdk-python/.env.local`. Re-uses the same
# `api-completion-examples` workspace that the other example projects
# share, so no duplicate seeding.
set -euo pipefail
