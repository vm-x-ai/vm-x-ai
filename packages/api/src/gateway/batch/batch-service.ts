...
export class CompletionBatchService {
  ...
  public async getById(
    ...
    const batch = await this.db.reader
    ...
    return batch
      ? (camelCaseEmbeds(batch, [
          'createdByUser',
          'createdByApiKey',
          'items',
        ]) as CompletionBatchDto)
      : undefined;
  }
  ...
  public async update(
    ...
    return row as CompletionBatchEntity;
  }
      ...
      .set(
        (eb) =>
          Object.entries(payload).reduce((acc, [key, value]) => {
            ...
            acc[column] = sql`${sql.ref(column)} + ${eb.lit(
              (value as number | null | undefined) ?? 0
            )}`;
            return acc;
  ...
  public async create(
    ...
    const batch = await this.db.writer.transaction().execute(async (tx) => {
      ...
      for (const item of items) {
        const itemEstimatedPromptTokens =
          this.tokenService.getRequestTokensFromChatCompletions(item.request);
    ...
    await this.completionBatchQueueService.sendBatch(
      batch,
